...
def main():
    ...
    dataset_path = DATA_DIR / "world_corpus.txt"
    if not dataset_path.exists():
        print(f"Error: {dataset_path} not found. Please run tools/harvest_internet.py first.")
        return

    print("Loading world corpus...")
    raw_text = dataset_path.read_text(encoding="utf-8", errors="ignore")
    
    # Take a massive chunk of data (up to 1 million characters) to push training to the limits
    raw_text = raw_text[:1000000]
    
    cleaned, _ = clean_text(raw_text)
    print(f"Loaded world dataset: {len(cleaned):,} characters")

    print("\n--- INITIALIZING PHARMAKON PINNACLE ARCHITECTURE ---")
    print("Dimensions: 128")
    print("Heads: 8")
    print("Layers: 4")
    print("---------------------------------------------------\n")
    
    # Initialize Scaled Up Model
    ...
    warmup = min(200, len(cleaned) // (32 * 64))
    print("Starting Hyper-Scale Training Run...")
    start = time.time()

    # Train (Using a larger batch size since we have more data)
    train_module.train(
        model=model, 
        data=cleaned, 
        char_to_idx=char_to_idx, 
        epochs=15, # 15 epochs on 1M chars is going to take several hours
        batch_size=32, 
        seq_len=64, 
        lr=3e-4, 
        weight_decay=0.01, 
